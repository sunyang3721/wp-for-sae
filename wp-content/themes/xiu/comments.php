...
<h3 class="title" id="comments">
	...
	<?php if( is_user_logged_in() ){ ?>
		<a href="<?php echo wp_logout_url() ?>"><span class="glyphicon glyphicon-user"></span> 退出</a>
	<?php }else{
		if ( !empty($comment_author) ) echo '<a href="javascript:;" data-event="comment-user-change"><span class="glyphicon glyphicon-user"></span> 更换用户</a>';
	} 
	?>
	</div>
	<strong><?php echo _hui('comment_title') ?> <?php echo hui_get_comment_number('','') ? '<b>'.hui_get_comment_number('','').'</b>':'<small>抢沙发</small>'; ?></strong>
</h3>
